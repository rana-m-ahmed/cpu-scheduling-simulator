...
function ParameterField({ label, value, onChange, min, max, step = 1 }) {
  return (
            ...
            {parameters.mlfqQuantums.map((quantum, index) => (
              <label key={`mlfq-${index}`} className="block space-y-2">
                ...
                <input
                  ...
                  className="w-full rounded border border-border bg-surface-1 px-3 py-2 font-mono text-text-primary outline-none transition focus:border-accent-cyan focus:ring-1 focus:ring-accent-cyan/20"
                />
              </label>
            ))}
          </div>
